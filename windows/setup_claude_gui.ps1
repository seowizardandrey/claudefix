# setup_claude_gui.ps1 - Configure Claude Desktop (GUI / MSIX) for local proxy
...
function Refresh-InternetSettings {
    try {
        [WinINetNotify]::InternetSetOption([IntPtr]::Zero, 39, [IntPtr]::Zero, 0) | Out-Null # INTERNET_OPTION_SETTINGS_CHANGED
        [WinINetNotify]::InternetSetOption([IntPtr]::Zero, 37, [IntPtr]::Zero, 0) | Out-Null # INTERNET_OPTION_REFRESH
    } catch {}
}

Write-Host "=========================================================" -ForegroundColor Cyan
Write-Host " Claude Desktop GUI Proxy Configuration" -ForegroundColor Cyan
Write-Host "=========================================================" -ForegroundColor Cyan

if ($Disable) {
    Write-Host "[gui] Disabling Windows proxy for Claude Desktop..." -ForegroundColor Yellow
    Set-ItemProperty -Path $RegPath -Name "ProxyEnable" -Value 0 -Type DWord
    Remove-ItemProperty -Path $RegPath -Name "AutoConfigURL" -ErrorAction SilentlyContinue
    Refresh-InternetSettings
    Write-Host "[gui] [OK] Proxy disabled. Direct connection restored." -ForegroundColor Green
    exit 0
}

# 1. Search for installed Claude Desktop (MSIX / Store package)
$claudeApp = Get-AppxPackage *Claude* -ErrorAction SilentlyContinue
if ($claudeApp) {
    $pkgFamily = $claudeApp.PackageFamilyName
    Write-Host "[gui] Found Claude Desktop MSIX package: $($claudeApp.Name) (v$($claudeApp.Version))" -ForegroundColor Green
    Write-Host "[gui] PackageFamilyName: $pkgFamily" -ForegroundColor Gray

    # Check Loopback Exemption
    $exemptions = CheckNetIsolation.exe LoopbackExempt -s
    if ($exemptions -notmatch [regex]::Escape($pkgFamily)) {
        Write-Host "[gui] Enabling localhost access for UWP container (LoopbackExempt)..." -ForegroundColor Yellow
        Write-Host "      Running CheckNetIsolation with admin privileges..." -ForegroundColor Gray
        try {
            $proc = Start-Process -FilePath "CheckNetIsolation.exe" -ArgumentList "LoopbackExempt -a -n=$pkgFamily" -Verb RunAs -Wait -PassThru
            if ($proc.ExitCode -eq 0) {
                Write-Host "      [OK] Loopback exemption successfully granted." -ForegroundColor Green
            } else {
                Write-Host "      Warning: CheckNetIsolation returned exit code $($proc.ExitCode)." -ForegroundColor Yellow
            }
        } catch {
            Write-Host "      UAC prompt was rejected or failed: $_" -ForegroundColor Red
        }
    } else {
        Write-Host "[gui] [OK] LoopbackExempt is already active for $pkgFamily." -ForegroundColor Green
    }
} else {
    Write-Host "[gui] Claude Desktop MSIX not found. Applying settings for standalone/classic app." -ForegroundColor Yellow
}

# 2. Configure WinINet System Proxy
Write-Host "`n[gui] Configuring Windows proxy settings for Claude Desktop..." -ForegroundColor Cyan
Write-Host "      Server: http=127.0.0.1:$ProxyPort;https=127.0.0.1:$ProxyPort" -ForegroundColor Gray

$bypassList = "localhost;127.0.0.1;<local>;*.ru;*.xn--p1ai;*.su;*.local;10.*;172.16.*;192.168.*"
Set-ItemProperty -Path $RegPath -Name "ProxyEnable" -Value 1 -Type DWord
Set-ItemProperty -Path $RegPath -Name "ProxyServer" -Value "http=127.0.0.1:$ProxyPort;https=127.0.0.1:$ProxyPort" -Type String
Set-ItemProperty -Path $RegPath -Name "ProxyOverride" -Value $bypassList -Type String
Remove-ItemProperty -Path $RegPath -Name "AutoConfigURL" -ErrorAction SilentlyContinue
Refresh-InternetSettings

Write-Host "      [OK] Windows proxy successfully activated (port $ProxyPort)." -ForegroundColor Green

# 3. Launch Claude Desktop
Write-Host "`n[gui] Launching Claude Desktop..." -ForegroundColor Cyan
$launched = $false

# Method A: Classic .exe
$classicExe = $null
if (Test-Path "$env:LOCALAPPDATA\Programs\Claude\Claude.exe") { $classicExe = "$env:LOCALAPPDATA\Programs\Claude\Claude.exe" }
elseif (Test-Path "C:\Program Files\Claude\Claude.exe") { $classicExe = "C:\Program Files\Claude\Claude.exe" }
elseif (Test-Path "${env:ProgramFiles(x86)}\Claude\Claude.exe") { $classicExe = "${env:ProgramFiles(x86)}\Claude\Claude.exe" }

if ($classicExe) {
    try {
        Start-Process -FilePath $classicExe -ArgumentList "--proxy-server=http://127.0.0.1:$ProxyPort"
        Write-Host "      [OK] Launched: $classicExe" -ForegroundColor Green
        $launched = $true
    } catch {}
}

# Method B: Direct executable inside MSIX package
if (-not $launched -and $claudeApp -and $claudeApp.InstallLocation) {
    $msixExes = Get-ChildItem -Path $claudeApp.InstallLocation -Filter "Claude.exe" -Recurse -ErrorAction SilentlyContinue
    foreach ($exe in $msixExes) {
        try {
            Start-Process -FilePath $exe.FullName -ArgumentList "--proxy-server=http://127.0.0.1:$ProxyPort"
            Write-Host "      [OK] Launched from MSIX: $($exe.FullName)" -ForegroundColor Green
            $launched = $true
            break
        } catch {}
    }
...
# Method C: UWP package protocol (shell:AppsFolder)
if (-not $launched -and $claudeApp) {
    try {
        Start-Process "explorer.exe" -ArgumentList "shell:AppsFolder\$($claudeApp.PackageFamilyName)!Claude"
        Write-Host "      [OK] Launched via Windows Apps: $($claudeApp.PackageFamilyName)" -ForegroundColor Green
        $launched = $true
    } catch {
        try {
            Start-Process "explorer.exe" -ArgumentList "shell:AppsFolder\$($claudeApp.PackageFamilyName)!App"
            Write-Host "      [OK] Launched via Windows Apps: $($claudeApp.PackageFamilyName)" -ForegroundColor Green
            $launched = $true
        } catch {}
    }
...
Write-Host " Claude Desktop GUI configured successfully!" -ForegroundColor Green
Write-Host "=========================================================" -ForegroundColor Green
Write-Host " Claude Desktop is now connected via local bridge (port $ProxyPort)." -ForegroundColor White
Write-Host " To disable Windows proxy at any time, run: .\setup_claude_gui.ps1 -Disable" -ForegroundColor Gray
Write-Host "=========================================================" -ForegroundColor Cyan

# SIG # Begin signature block
# MIIcyAYJKoZIhvcNAQcCoIIcuTCCHLUCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUAB