...
Write-Host " Остановка служб Claude Code Proxy (Windows)" -ForegroundColor Cyan
Write-Host "=========================================================" -ForegroundColor Cyan
...
if ($bridgeProcs) {
    foreach ($p in $bridgeProcs) {
        Write-Host "[stop] Завершение процесса моста (PID: $($p.ProcessId))..." -ForegroundColor Yellow
        Stop-Process -Id $p.ProcessId -Force -ErrorAction SilentlyContinue
    }
    Write-Host "[stop] Прокси-мост остановлен." -ForegroundColor Green
} else {
    Write-Host "[stop] Процессы прокси-моста не найдены." -ForegroundColor Gray
}

# 2. Stop watcher
$watcherProcs = Get-CimInstance Win32_Process -Filter "CommandLine LIKE '%claude_watcher.ps1%'" -ErrorAction SilentlyContinue
if ($watcherProcs) {
    foreach ($p in $watcherProcs) {
        Write-Host "[stop] Завершение процесса автопатчера (PID: $($p.ProcessId))..." -ForegroundColor Yellow
        Stop-Process -Id $p.ProcessId -Force -ErrorAction SilentlyContinue
    }
    Write-Host "[stop] Автопатчер остановлен." -ForegroundColor Green
} else {
    Write-Host "[stop] Процессы автопатчера не найдены." -ForegroundColor Gray
}

# 3. Disable AutoStart if requested
if ($DisableAutoStart) {
    ...
    Write-Host "[stop] Автозапуск отключен (ярлык удален из Автозагрузки)." -ForegroundColor Yellow
}

Write-Host "=========================================================" -ForegroundColor Cyan
Write-Host " Все службы Claude Code остановлены." -ForegroundColor Green
Write-Host "=========================================================" -ForegroundColor Cyan

# SIG # Begin signature block
# MIIcyAYJKoZIhvcNAQcCoIIcuTCCHLUCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDoUN4J7JWLc7+H
# RvgtzrvBdZK6KtOhIV/w3u9qLzBsOaCCFs4wggOQMIICeKADAgECAhA+YjK6lN32
...
# BgkqhkiG9w0BCQQxIgQgLl6UyThvL5N+xWu1KDabBhf/M3ysDVmjqwu0Ua8fJsAw
# DQYJKoZIhvcNAQEBBQAEggEAG4gXhlQodp5eKXUITF7bowKe5h7K+Uq6OOde+0bF
# w6yLLv7GHPcp/QONsrfXH1WtYqdUbaixxQzv74ahSkMviOY9j0KJFOQDTWoCIKW6
# ZGQMsCJQCosqtzV2kVx6q4xMfC7/COEcjy5IPhYEEIkLYIBrIiOOm8WofY0H1JIV
# kpmlWnvjKsxe7/4FI72lAaTtoWY0/heCRIt0y2ByXZ5veYN/t7nGiUd8jpDAFPxt
# O/Le5KVxpO+hJMydCaRQ/Q4R1PutyRXI8D6XFMXIYbv7V2Ry6DVL8TpsAM09rQK8
# MYvVxyDf/8TQ1qU5seVJWmHVVq46y7Jbfs5DwML/zD/OqqGCAyYwggMiBgkqhkiG
...
# SIb3DQEJBTEPFw0yNjA5MjIxMzE5NTdaMC8GCSqGSIb3DQEJBDEiBCCqmmiZHBYt
# c5qtCLKfiPuP7nESjxtTy3f/8WsgVHtPvTANBgkqhkiG9w0BAQEFAASCAgBxwe+b
# H/NF/UoN0trUn1QBF4Qe5VUPXQ1IJHakH9/RtRnagd+MpYguZrOZK5x5uN2XvxiL
# OrCWkzHLKY9m7YB78UUsVEc9bvTO2VA/T6yDNRd3M4P/Vi/OpNuBxOKX7/sxT8Gm
# 4ZJP/vqOL0UTezO/pELlk37kfZ0XsETdv2dVjve0/pw00LbvbMhWjV+87fwEcLPM
# 3x6FCrSc9zqHB/s5QZslIWOIzm+pxBgYaz6MWlkssbiMe4reJ0PO6nQtVLKd8Iat
# zUrkqEO70CxG+q3T3qH3gQxjQadR3n9EGOlP5DUlEx3uZf1qwH1Z+kWZ/i9mxKhA
# BZ46oNN619QTM5Zu/ELkb7+bDAwTUMjpMBLsA4ODlelY9US5noUv7npkjiENrEo7
# irZkssrGv8uiRXCS5c5rx/kw6Dg99jwBunUh4p4ZUztZoZye9FkkPl1ZrfVU8/vP
# 3RiOra9lTQkxC8XxbKu47eozmLD0WX6msjo+l6w/lh77P0QKZ+Z4MVPaWxc+QTQY
# VK3Y0JTUcxvmiYcJcMq+QX57Vcgb9qSyLvKtO14gXiV84aJGSzhQHmjzZvRkF2NA
# huzvVJ44PDunJSM+eIB3r4IwVJ4VBMBpyAo4ga3mjMJSUEZ/Rf7zb6KUIFXsAMw2
# wlPypQswBcjbvQOLtoWkgh/7XW2uFZV+UhmtNg==
