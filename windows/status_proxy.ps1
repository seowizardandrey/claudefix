# status_proxy.ps1 - Проверка статуса служб и диагностики Claude Code Proxy (Windows)
...
Write-Host " Статус служб Claude Code Proxy (Windows)" -ForegroundColor Cyan
Write-Host "=========================================================" -ForegroundColor Cyan
Write-Host " Локальный порт: $ProxyPort" -ForegroundColor Gray
Write-Host " Конфигурация  : $ConfigFile" -ForegroundColor Gray
...
if ($isListening) {
    Write-Host "[СЕТЬ]      Локальный порт $ProxyPort : СЛУШАЕТ (АКТИВЕН)" -ForegroundColor Green
} else {
    Write-Host "[СЕТЬ]      Локальный порт $ProxyPort : НЕ СЛУШАЕТ (ОСТАНОВЛЕН)" -ForegroundColor Red
}

# 2. Проверка процессов
$bridgeProcs = Get-CimInstance Win32_Process -Filter "CommandLine LIKE '%claude_proxy_bridge%'" -ErrorAction SilentlyContinue
if ($bridgeProcs) {
    $pids = ($bridgeProcs | ForEach-Object { $_.ProcessId }) -join ", "
    Write-Host "[МОСТ]      Процесс моста           : РАБОТАЕТ (PID: $pids)" -ForegroundColor Green
} else {
    Write-Host "[МОСТ]      Процесс моста           : НЕ ЗАПУЩЕН" -ForegroundColor Red
}

$watcherProcs = Get-CimInstance Win32_Process -Filter "CommandLine LIKE '%claude_watcher%'" -ErrorAction SilentlyContinue
if ($watcherProcs) {
    $pids = ($watcherProcs | ForEach-Object { $_.ProcessId }) -join ", "
    Write-Host "[АВТОПАТЧ]  Процесс наблюдателя     : РАБОТАЕТ (PID: $pids)" -ForegroundColor Green
} else {
    Write-Host "[АВТОПАТЧ]  Процесс наблюдателя     : НЕ ЗАПУЩЕН" -ForegroundColor Yellow
}

# 3. Проверка автозапуска
Write-Host "`n[АВТОЗАПУСК]:" -ForegroundColor Cyan

# Проверка Планировщика Задач
$task = Get-ScheduledTask -TaskName "ClaudeProxyBridge" -ErrorAction SilentlyContinue
if ($task) {
    Write-Host "  - Планировщик Windows (Task Scheduler) : АКТИВЕН ($($task.State))" -ForegroundColor Green
} else {
    Write-Host "  - Планировщик Windows (Task Scheduler) : Не зарегистрирован" -ForegroundColor Gray
}

# Проверка реестра Run
$regRun = Get-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run" -Name "ClaudeProxyBridge" -ErrorAction SilentlyContinue
if ($regRun) {
    Write-Host "  - Реестр Windows (HKCU Run)            : АКТИВЕН" -ForegroundColor Green
} else {
    Write-Host "  - Реестр Windows (HKCU Run)            : Не зарегистрирован" -ForegroundColor Gray
}

# Проверка папки Автозагрузка
$startupDir = [Environment]::GetFolderPath([Environment+SpecialFolder]::Startup)
$vbsPath = Join-Path $startupDir "ClaudeProxyStartup.vbs"
$lnkPath = Join-Path $startupDir "ClaudeProxy.lnk"
if ((Test-Path $vbsPath) -or (Test-Path $lnkPath)) {
    Write-Host "  - Папка Автозагрузка (Startup)         : АКТИВЕН" -ForegroundColor Green
} else {
    Write-Host "  - Папка Автозагрузка (Startup)         : Отсутствует" -ForegroundColor Gray
}

# 4. Проверка логов ошибок
$errLog = Join-Path $ConfigDir "bridge_err.log"
if (Test-Path $errLog) {
    $errContent = Get-Content -Path $errLog -Tail 10 -ErrorAction SilentlyContinue
    if ($errContent) {
        Write-Host "`n[ПОСЛЕДНИЕ ОШИБКИ ИЗ $errLog]:" -ForegroundColor Red
        $errContent | ForEach-Object { Write-Host "  $_" -ForegroundColor Red }
    }
}

$outLog = Join-Path $ConfigDir "bridge.log"
if (Test-Path $outLog) {
    $outContent = Get-Content -Path $outLog -Tail 5 -ErrorAction SilentlyContinue
    if ($outContent) {
        Write-Host "`n[ПОСЛЕДНИЕ СООБЩЕНИЯ ИЗ $outLog]:" -ForegroundColor Yellow
        $outContent | ForEach-Object { Write-Host "  $_" -ForegroundColor Gray }
    }
}

Write-Host "=========================================================" -ForegroundColor Cyan
if (-not $isListening) {
    Write-Host "Для ручного старта выполните: .\start.cmd или .\start_proxy.ps1" -ForegroundColor Yellow
}

# SIG # Begin signature block
# MIIcyAYJKoZIhvcNAQcCoIIcuTCCHLUCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCuCCMdd93sp0Be
# FkDzNg0rZNR87DuSc/S1j807mEaULqCCFs4wggOQMIICeKADAgECAhA+YjK6lN32
...
# BgkqhkiG9w0BCQQxIgQgC+RUqlgVzVbrja8FmWKXvApLM/Yaie785OXWZmB0x7Uw
# DQYJKoZIhvcNAQEBBQAEggEAlb1wnTS/XEVkqP6AGuSygogx4BKeeRJ/XK5sFjTC
# C2NUfRDVgJPC+uX7AI2wb6d6739MPbqfgrUVFAKSXV8LNcHdqk4M7GIlt0oLwEBP
# ewVdvTPjenxqCe8uw05SNp6Kkdk7l26hG3REeqirbzJVhAYaP5xIpKB7+xI9d3wQ
# dDrj1j+kiGGBThVWZCEh97YbGZe6VqjUsIQu6fiCkJWjQRoNC2X7zeuL7chCNWx4
# V+GVa9WvDHSb5J6dd71SWzPeqgkVBLiuGt++/u1koD5pfbZ+xcRZwYOa6zA68pGK
# /guuDjFsK7am1wkSFsydojCEVsrm+J8NmqNnxyprLsr7A6GCAyYwggMiBgkqhkiG
...
# SIb3DQEJBTEPFw0yNjA5MjIxMzE5NTdaMC8GCSqGSIb3DQEJBDEiBCAeoWzJPV7w
# aAts357S2bwQhp45nZtX0QU32p9ny+0SLTANBgkqhkiG9w0BAQEFAASCAgBhj/Kn
# 8OAOZWCh+fp2ZrcdzamCX0Ie3qQEQAK3Yd1LPDuukG6QdG9TJwL+/lkOgbnQ2lGu
# a2wBLUSoK1WrFQtjdxAwuVdt1axhEcILb5gVFHRmiUX3qLGt/ObTp8DS6gEMwbBg
# MDmq8pS6q6/ePmZim1b+k1laV5t+fDC0fZ9rrzKlcagV0x2EPXMNKweJIZw7w9RM
# +pvRDSyEIhIAe+sMDjoNVHUq+g1UrLAcfRV1xIUvpV9UzUt4TINxFUmbMaXUBfvs
# aCzCO7vzgbM6+atWBxW/NnUX/IF+H/LTYHnwdEBD4r8oPFsxfpHGLP6/g8esuS0x
# hzeUa9Q1XPK+/Bn7UDNZNKli2JbE9MqM/kr0hzzgtUFfkJgPOQTIILKEZSU55Ecg
# /BcJrhSkml+CxTwxwYgA25QnoyFgz20h2DVd20DVjWoPJBH/hl28+z5Qi8ACr5Bw
# u8geb87LsZol2FZ2cn69FZiJTp1PLM2CLNdd8kEoXc5B95A92ghDd0VzO5YNacDf
# Z4EI8ntYvQOOE29X0MIOD6QnUKiM4aVRgJwA2is2GDDXs8aFPPNq54n8GJyRAsRz
# U8LPqjLpC82v8V+hWyodiKA9FO+y5aMweM3R5x8bhi96UfTrBleZpiNrQt4c8LDU
# w78vdV2+fkf3ju4PFD9X+iaNoAfwvJJdWYli0A==
